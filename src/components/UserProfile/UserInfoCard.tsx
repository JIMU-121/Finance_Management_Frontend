  ...
  return (
    <div>
      ...
      <p className="text-sm font-medium text-gray-800 dark:text-white/90">
        {value || (
          <span className="italic text-gray-400 dark:text-gray-600">Not provided</span>
        )}
      </p>
    </div>
  );
...
export default function UserInfoCard() {
  ...
  const handleOpen = () => {
    // Re-sync form state from latest AuthContext values (already refreshed by the page on mount)
    setForm({
      firstName: user?.firstName ?? "",
      lastName: user?.lastName ?? "",
      username: user?.username ?? "",
  ...
  const handleSave = async () => {
    ...
    try {
      await patchUser(Number(user.id), {
        ...
        // Convert string → int enum expected by the backend (UserGender)
        ...(form.gender !== "" && { gender: genderToEnum[form.gender] }),
      });
      // Fetch fresh data from DB and update the context so all cards re-render
            ...
            <InfoRow
              ...
              value={
                user?.gender !== undefined
                  ? (genderFromEnum[user.gender] ?? null)
                  : null
              }
            />
          </div>
